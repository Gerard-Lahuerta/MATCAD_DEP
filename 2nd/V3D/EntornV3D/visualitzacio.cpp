...
// Dibuix de les cares back com a línies en Il.luminacio PLANA i SUAU
		...
		if (bc_lin) {	if (frnt_fcs) glPolygonMode(GL_BACK, GL_LINE);
...
// -------- Entorn V3D: ORTOGRÀFICA (Funcions Projeccio_Orto i Vista_Ortografica)

// Projeccio_Orto: Definició Viewport i glOrtho 
// ---- Entorn V3D: ATENCIÓ!!. CAL DEFINIR PARÀMETRES DE LA FUNCIÓ
void Projeccio_Orto(int minx, int miny, GLsizei w, GLsizei h, float zoom)
{   
// ---- Entorn V3D: ATENCIÓ!!. ESPECIFICACIO DELS PARÀMETRES DE PROJECCIÓ ORTOGRÀFICA
//			        QUE ES CARREGUEN A LA MATRIU DE PROJECCIÓ GL_PROJECTION

	// Definició Viewport
	

	glViewport(minx, miny, w, h); //minx = lower left corner = miny. w = anchura. h = altura --> establim el marc on dibuixarem
	//glViewport(0, 0, 150, 200);
	...
	// PROJECCIO ORTOGRAFICA.Definim volum de visualització adaptant-lo 
	//	 a les mides actuals de la finestra windows	

	// Amb gluPerspective
	if (w >= h) glOrtho(-100 * w / h, 100 * w / h, -100, 100, 0, 1000); // 1r-> fovy = angulo en grados del campo de visión en el eje de las y. 2n -> aspect = determina el camp de visió en la direcció x. 3r -> zNear = distancia des del viewer fins al principi del punt de visió (>0). 4t -> zFar = distancia des del viewer fins al final del punt de visio (>0)
	else glOrtho(-100, 100, -100 * h/w, 100 * h/w, 0, 1000);

	//Amb glOrtho
	//glOrtho(-w/32, w/32, -h/32, h/32, p_near, p_far + zoom  );
...
//								tipus de projecció definit a la variable prj.
	switch (prj) {
		...
		case 3:
			gluLookAt(200, 200, 200, 0,0,0, -1,-1,1);
			break;
	}
...
// -------- Entorn V3D: PERSPECTIVA (Funcions Projeccio_Perspectiva i Vista_Esferica)
...
void instancia(bool TR, INSTANCIA tg, INSTANCIA tgF, bool rotQ, bool animQ, GLdouble Qmat[16], CPunt3D intTr)
{
	if ((!rotQ) && (!animQ))
		...
		else { // Entorn V3D. QUATERNIONS:	// Orientació objecte per quaternions i traslació interpolada en animació dins mode Quaternions.
				glTranslatef(intTr.x, intTr.y, intTr.z);
				glMultMatrixd(Qmat);	// Matriu Rotacions del Quaternió	
			}
}
...
// Eix X
	...
	for (i = -size; i < size + 1; i++)
		{	glBegin(GL_LINES);
			  glVertex3f(i, h, -size);	 // glVertex3f(i, -0.5f, 0.0f);
			  glVertex3f(i, h, +size); // glVertex3f(i, +0.5f, 0.0f);
			glEnd();
		}
...
// Eix Z
	...
	for (i = -size; i < size + 1; i++)
		{	glBegin(GL_LINES);
			  glVertex3f(-size, h, i);		// glVertex3f(0.0, -0.5f, i);
			  glVertex3f(size, h, i);		// glVertex3f(0.0f, 0.5f, i);
			glEnd();
	}
}
...
// Assignació automàtica de coordenades textura
...
//	glTexGeni(GL_S, GL_TEXTURE_GEN_MODE, GL_SPHERE_AMP);
